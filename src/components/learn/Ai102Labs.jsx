...
import {
  getDomainLabel,
  labDomainToId,
} from "../../utils/ai102DomainClassifier";
...
export function Ai102Labs({ cert }) {
  ...
  const discoveredLinks = useMemo(() => {
    const known = new Set(
      [...OFFICIAL_LABS, ...LEGACY_LABS].map((lab) => normalizeUrl(lab.url)),
    );
    ...
    cert.questions.forEach((q) => {
      ...
      extractUrls(pool).forEach((url) => {
        if (
          url.includes("microsoftlearning.github.io") ||
          url.toLowerCase().includes("/labs/")
        ) {
  ...
  const labs = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(
        (lab) => (statusByUrl[lab.url] ?? "not-started") === statusFilter,
      );
    }
    ...
    <section className="animate-slide-up space-y-6">
      ...
      <div className="panel flex flex-col gap-4 p-5 md:flex-row md:items-end md:justify-between">
        <div className="w-full md:w-1/2">
          <label
            className="text-xs font-bold uppercase text-muted dark:text-slate-400"
            htmlFor="ai102-labs-search"
          >
            Search labs
          </label>
          <input
      ...
      <div className="grid gap-3 md:grid-cols-4">
        <div className="panel p-5">
          <p className="text-xs font-bold uppercase text-muted dark:text-slate-400">
            Total labs
          </p>
          <p className="mt-2 text-3xl font-black text-ink dark:text-slate-100">
            {stats.total}
          </p>
        </div>
        <div className="panel p-5">
        ...
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-amber-900/80 dark:text-amber-100/80">
              Microsoft lists AI-102 as retiring on June 30, 2026. The old
              AI-102-AIEngineer repo is archived, so this page prioritizes
              current MicrosoftLearning mslearn-* labs and keeps a small
              archived section only for classic vision coverage.
            </p>
          </div>
        </div>
      </div>
        ...
        {SOURCE_LINKS.map((source) => (
          <article className="panel p-5" key={source.url}>
            <div className="flex items-start justify-between gap-3">
              ...
              <a
                className="secondary-button !min-h-9 shrink-0"
                href={source.url}
                target="_blank"
          ...
          <section className="space-y-3" key={level}>
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <h2 className="text-2xl font-black text-ink dark:text-slate-100">
                  {level}
                </h2>
              </div>
              <p className="text-sm font-semibold text-muted dark:text-slate-400">
                {levelLabs.length} labs
              </p>
            </div>
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`rounded-full px-2.5 py-1 text-xs font-bold ${FIT_TONE[lab.fit]}`}
                        >
                          {lab.fit} AI-102 fit
                        </span>
                      ...
                      <h3 className="mt-3 text-lg font-black text-ink dark:text-slate-100">
                        {lab.title}
                      </h3>
                      <p className="mt-1 text-sm font-semibold text-accent dark:text-sky-300">
                        {lab.domain}
                      </p>
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-muted dark:text-slate-400">
                        <span className="rounded-md bg-slate-100 px-2 py-1 dark:bg-slate-800">
                          Domain ID:{" "}
                          <span className="font-semibold">{lab.domainId}</span>
                        </span>
                        <span className="rounded-md bg-slate-100 px-2 py-1 dark:bg-slate-800">
                          Nguồn:{" "}
                          <span className="font-semibold">{lab.source}</span>
                        </span>
                      </div>
                    ...
                    <a
                      className="secondary-button !min-h-10 shrink-0"
                      href={lab.url}
                      target="_blank"
                      rel="noopener noreferrer"
                    >
